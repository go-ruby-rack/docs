...
Contributions are welcome. `go-ruby-rack/rack` is built to a small set of
...
- **Differential testing against the `rack` gem.** Correctness is defined by
  reference Ruby. A corpus is run through both `ruby` (with the `rack` gem) and
  this library and the results are compared byte-for-byte — not approximated from
  memory.
...
- **A reusable library, not the server.** This module implements the
  deterministic compute core extracted from rbgo's internals. Anything that needs
  a live Ruby binding or an HTTP server belongs in the consumer, not here.
...
The library is in
[`github.com/go-ruby-rack/rack`](https://github.com/go-ruby-rack/rack). This
documentation site is in
[`github.com/go-ruby-rack/docs`](https://github.com/go-ruby-rack/docs). Start
